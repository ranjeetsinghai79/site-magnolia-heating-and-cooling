...
export async function runImageGeneratorAgent(
  ...
  // ── Priority 3: AI-generated (fal.ai Flux Pro or HuggingFace) ──────────────
  if (!falKey && !hfToken) {
  ...
  try {
    ...
    // Count how many fal.ai images succeeded (HuggingFace is free, don't log)
    const falCount = Object.values(images).filter(Boolean).length
    if (falCount > 0 && process.env.FAL_KEY) {
      await logCost({ service: 'fal_images', units: falCount, leadId: lead.id, leadName: lead.name })
    }
